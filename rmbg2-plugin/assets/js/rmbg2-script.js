/**
 * RMBG2 - Background Remover Plugin v1.0.0
 * Uses BriaAI/RMBG-2.0 for state-of-the-art client-side background removal
...
const DEBUG = true;
function log(...args) {
    if (DEBUG) console.log('[RMBG2]', ...args);
}
function logError(...args) {
    console.error('[RMBG2 ERROR]', ...args);
}
...
let env, AutoModelForImageSegmentation, AutoProcessor, RawImage;
...
try {
    ...
    log('AutoModelForImageSegmentation available:', !!AutoModelForImageSegmentation);
} catch (importError) {
    logError('Failed to import transformers.js:', importError);
    throw importError;
}

// Configuration - using quantized model for better browser performance
const MODEL_ID = 'briaai/RMBG-2.0';
const CACHE_KEY = 'rmbg2_model_loaded_v1';
...
const LOADING_TIPS = [
    ...
    "RMBG-2.0 is the latest state-of-the-art model from BriaAI",
    "Built on BiRefNet architecture for superior quality",
    "The AI model runs entirely in your browser",
    "Works offline after the first load",
    "No sign-up or account required",
    "Processing happens on your device, not our servers",
    "RMBG-2.0 excels at complex edges like hair and fur",
    "Trained on 15,000+ professional-grade images"
];
...
function init() {
    ...
    // Cache DOM elements
    elements = {
        container: document.getElementById('rmbg2-container'),
        firstTimeNotice: document.getElementById('rmbg2-first-time-notice'),
        modelStatus: document.getElementById('rmbg2-model-status'),
        loaderLabel: document.getElementById('rmbg2-loader-label'),
        loaderPercent: document.getElementById('rmbg2-loader-percent'),
        progressFill: document.getElementById('rmbg2-progress-fill'),
        modelHint: document.getElementById('rmbg2-model-hint'),
        uploadArea: document.getElementById('rmbg2-upload-area'),
        fileInput: document.getElementById('rmbg2-file-input'),
        processing: document.getElementById('rmbg2-processing'),
        processingText: document.getElementById('rmbg2-processing-text'),
        processingProgress: document.getElementById('rmbg2-processing-progress'),
        results: document.getElementById('rmbg2-results'),
        originalImage: document.getElementById('rmbg2-original-image'),
        resultImage: document.getElementById('rmbg2-result-image'),
        downloadBtn: document.getElementById('rmbg2-download-btn'),
        newImageBtn: document.getElementById('rmbg2-new-image-btn'),
        error: document.getElementById('rmbg2-error'),
        errorMessage: document.getElementById('rmbg2-error-message')
    };
    ...
    if (!elements.container) {
        logError('Container not found! Make sure [rmbg2] shortcode is on the page.');
        return;
    }
...
async function preloadModel() {
    ...
    elements.progressFill.classList.remove('rmbg2-progress-indeterminate');

    try {
        log('Loading RMBG-2.0 model with AutoModelForImageSegmentation...');

        // Load model - RMBG-2.0 uses AutoModelForImageSegmentation
        // Using quantized int8 model for better browser performance
        model = await AutoModelForImageSegmentation.from_pretrained(MODEL_ID, {
            device: 'wasm',
            dtype: 'q4', // Use quantized model for browser
            progress_callback: (progress) => {
                ...
                if (progress.status === 'progress') {
                    ...
                    if (progress.file && progress.file.includes('onnx')) {
                        ...
                        elements.progressFill.classList.add('rmbg2-progress-indeterminate');
                    }
        ...
        if (error.message.includes('Unsupported model type')) {
            errorMsg = 'RMBG-2.0 requires a newer version of the runtime. Please try again later or use RMBG-1.4 instead.';
        } else if (error.message.includes('memory')) {
            errorMsg = 'Not enough memory to load RMBG-2.0. Try closing other tabs or use RMBG-1.4 for lower memory usage.';
        }

        showError('Failed to load AI model: ' + errorMsg);
        elements.modelStatus.style.display = 'none';
    } finally {
...
function handleDragOver(e) {
    e.preventDefault();
    e.stopPropagation();
    elements.uploadArea.classList.add('rmbg2-dragover');
}

function handleDragLeave(e) {
    e.preventDefault();
    e.stopPropagation();
    elements.uploadArea.classList.remove('rmbg2-dragover');
}
...
function handleDownload() {
    ...
    link.download = 'background-removed-rmbg2-' + Date.now() + '.png';
    link.href = resultSrc;
